...
def check_validity(scores: dict) -> dict:
    ...
    if scores["cannot_say"] >= 30:
        ...
        flags.append(f"CAUTION: {scores['cannot_say']} items omitted (Cannot Say). Interpret with caution. "
                     f"If items fall primarily after item 370, Validity and Clinical Scales may still be interpreted "
                     f"but Content, Supplementary, and Harris-Lingoes Scales should not.")

    # ── VRIN ────────────────────────────────────────────────────
    vrin_t = scores["VRIN_T"]
    if vrin_t >= 80:
        flags.append(f"INVALID: VRIN T={vrin_t} — inconsistent random responding detected. "
                     f"Profile is invalid and should not be interpreted.")
        valid = False
    elif vrin_t >= 70:
        flags.append(f"CAUTION: VRIN T={vrin_t} — possible inconsistent responding. "
                     f"Interpret profile with caution; consider re-administration.")

    # ── TRIN ────────────────────────────────────────────────────
    trin_t = scores["TRIN_T"]
    if trin_t >= 80:
        flags.append(f"CAUTION: TRIN T={trin_t} — acquiescence or counter-acquiescence response set detected. "
                     f"Client may have answered True (or False) indiscriminately. "
                     f"Review response pattern before interpreting clinical scales.")
    elif trin_t >= 70:
        flags.append(f"NOTE: TRIN T={trin_t} — suspect response set; possible tendency toward uniform True or False responding.")

    # ── F Scale ─────────────────────────────────────────────────
    f_t = scores["F_T"]
    if f_t >= 100:
        flags.append(f"CAUTION: F T={f_t} — extremely elevated. Consider: random responding, faking bad, "
                     f"or acute severe psychopathology (delusions, hallucinations, psychotic disorder). "
                     f"Profile may be invalid unless VRIN is in acceptable range and client is a known psychiatric inpatient.")
    elif f_t >= 80:
        flags.append(f"CAUTION: F T={f_t} — elevated. Possible symptom exaggeration, plea for help, "
                     f"resistance to testing, or genuine severe psychopathology. "
                     f"Interpret clinical scales cautiously; consider malingering evaluation.")
    elif f_t >= 65:
        flags.append(f"NOTE: F T={f_t} — mildly elevated. May reflect genuine distress, deviant attitudes, "
                     f"or endorsement of specific problem areas. Clinical scales are likely interpretable.")

    # ── Fb Scale ────────────────────────────────────────────────
    fb_t = scores["Fb_T"]
    if fb_t >= 110:
        flags.append(f"INVALID: Fb T={fb_t} — invalid protocol, especially if F is also ≥110. "
                     f"Do not interpret Content or Harris-Lingoes Scales.")
    elif fb_t >= 89:
        if f_t >= 89:
            flags.append(f"CAUTION: Fb T={fb_t} — do not interpret Content or Harris-Lingoes Scales.")
        else:
            flags.append(f"NOTE: Fb T={fb_t} — elevated back-half infrequency. "
                         f"Consider random responding, exaggeration, or fatigue in latter portion of test.")

    # ── Fp Scale ────────────────────────────────────────────────
    fp_t = scores["Fp_T"]
    if fp_t >= 100:
        flags.append(f"CRITICAL: Fp T={fp_t} — extremely elevated Infrequency-Psychopathology scale. "
                     f"Items rarely endorsed even by genuine psychiatric inpatients. "
        ...
        flags.append(f"CAUTION: Fp T={fp_t} — significantly elevated. Items on this scale are rarely endorsed "
                     f"even by patients with confirmed severe mental illness. "
                     f"Possible malingering or dramatic symptom overreporting. "
                     f"Interpret clinical profile with significant caution.")
    elif fp_t >= 70:
        flags.append(f"NOTE: Fp T={fp_t} — mildly elevated. May reflect genuine unusual experiences "
                     f"or some degree of symptom exaggeration. Consider alongside F and VRIN.")

    # ── L Scale ─────────────────────────────────────────────────
    l_t = scores["L_T"]
    if l_t >= 65:
        flags.append(f"NOTE: L T={l_t} — overly virtuous, unsophisticated defensive presentation. "
                     f"Client is denying minor flaws most people admit. "
                     f"Profile with L ≥65 should be interpreted cautiously; possible faking good.")
    elif l_t >= 60:
        flags.append(f"NOTE: L T={l_t} — mildly elevated; more conforming than average, tendency toward denial.")

    # ── K Scale ─────────────────────────────────────────────────
    k_t = scores["K_T"]
    if k_t >= 65:
        flags.append(f"NOTE: K T={k_t} — defensive responding; possible underreporting of psychological distress. "
                     f"Client may be attempting to appear adequate and in control.")
    elif k_t <= 40:
        flags.append(f"NOTE: K T={k_t} — low defensiveness; client is openly acknowledging significant distress. "
                     f"May reflect genuine severe disturbance or exaggerated self-disclosure.")

    # ── S Scale ─────────────────────────────────────────────────
    s_t = scores["S_T"]
    if s_t <= 38:
        flags.append(f"NOTE: S T={s_t} — low Superlative Self-Presentation. "
                     f"Client is not presenting in an overly positive or superlative manner. "
                     f"May reflect openness about problems or significant distress.")
    elif s_t >= 70:
        flags.append(f"NOTE: S T={s_t} — elevated Superlative Self-Presentation. "
                     f"Client endorsing exceptionally positive self-image across multiple domains "
                     f"(serenity, contentment, patience, moral integrity). "
                     f"Consider defensiveness or highly favorable life circumstances.")

    # ── F-K Index ───────────────────────────────────────────────
    fk = scores["FK_index"]
    if fk > 12:
        flags.append(f"CAUTION: F-K index = {fk} (>12) — strong indicator of possible symptom exaggeration or faking bad.")
    elif fk > 9:
        flags.append(f"NOTE: F-K index = {fk} (>9) — possible symptom exaggeration. Use conservative cutoff of 12 for definitive conclusions.")
    elif fk < -12:
        flags.append(f"CAUTION: F-K index = {fk} (<-12) — strong indicator of possible symptom minimization or faking good.")
    elif fk < -9:
    ...
    if len(suicidal_items) >= 3:
        flags.append(f"⚠ RISK ALERT: {len(suicidal_items)} Koss-Butcher Depressed Suicidal Ideation critical items endorsed "
                     ...
                     f"This finding must be explicitly addressed in the clinical report.")
...
def generate_report(client_name: str, age: str, gender: str, scores: dict, validity: dict,
                    dob: str = "Not provided", nationality: str = "Not provided",
                    referral: str = "Not provided") -> str:
    clinical = {
...
- S ≥70: Highly positive self-presentation across serenity, contentment, and moral dimensions
...
Scale 1 (Hs) Hypochondriasis — 32 items:
  T≥65: Excessive bodily concerns; vague somatic complaints; pessimistic; selfish/narcissistic; poor therapy candidate; resists psychological interpretations
  T 58–64: Some somatic concern; immature and stubborn
  T 40–57: Normal
  T<40: Free of somatic preoccupation; alert and insightful

Scale 2 (D) Depression — 57 items:
  T≥65: Depressive symptoms; blue/dysphoric; pessimistic; guilt; psychomotor retardation; introverted; good therapy candidate but may terminate when crisis passes
  T 58–64: Dissatisfied but may not recognize as depression
  T 40–57: Normal
  T<40: Relaxed; self-confident; may be impulsive

Scale 3 (Hy) Hysteria — 60 items:
  T≥65: Reacts to stress via physical symptoms; immature/childish; self-centered; lacks insight; responds well to direct advice; rarely reports hallucinations or delusions
  T 58–64: Exhibitionistic; naive; denies problems
  T<40: Constricted; cold; suspicious

Scale 4 (Pd) Psychopathic Deviate — 50 items:
  T≥65: Difficulty incorporating social values; rebellious; impulsive; poor judgment; narcissistic; superficial relationships; poor therapy prognosis (especially T>75)
  T 58–64: May reflect situational conflict or genuine social concern
  T<40: Conventional; conforming; passive

Scale 5 (Mf) Masculinity-Femininity — 50 items:
  Males T≥65: Aesthetic/artistic interests; sensitive; androgynous; passive; NOT indicative of homosexuality
  Males T<40: Stereotypically masculine; aggressive; action-oriented; inflexible
  Note: Scale 5 is typically NOT interpreted clinically in isolation

Scale 6 (Pa) Paranoia — 40 items:
  T≥70: Frankly paranoid; delusions of persecution/grandeur; ideas of reference; receives schizophrenia or paranoid disorder diagnoses
  T 60–70: Paranoid predisposition; overly sensitive; suspicious; guarded; rationalization; poor therapy prognosis
  T 50–60: Paranoid orientation; very sensitive to others' opinions
  T 35–50: Normal
  T<35: Insensitive; evasive; may be shy or secretive

Scale 7 (Pt) Psychasthenia — 48 items:
  T≥85: Agitated ruminations and obsessions no longer controlling anxiety — extreme distress
  T≥65: Psychological turmoil; anxious/tense; obsessive/compulsive; introspective; insecure; rigid/moralistic; good therapy motivation but slow progress; not responsive to brief therapy
  T 58–64: Punctual; not seen as anxious
  T<40: Free of disabling anxiety; self-confident; emotionally stable

Scale 8 (Sc) Schizophrenia — 78 items:
  T≥91: Acute severe situational stress; identity crisis; typically NOT schizophrenic
  T 65–90: Possible thought disorder (especially approaching T=80); confused/disorganized; schizoid lifestyle; isolated/alienated; withdrawn; poor therapy prognosis
  T 58–64: Thinks differently; creative or schizoid-like
  T<40: Friendly; well-balanced; submissive; cautious

Scale 9 (Ma) Hypomania — 46 items:
  T≥80: Behavioral manifestations of manic episode (excessive activity, flight of ideas, hallucinations, grandiosity)
  T 65–79: Overactive; unrealistic self-appraisal; energetic; low frustration tolerance; poor therapy prognosis
  T 58–64: Active and outgoing; may find restrictions frustrating
  T<40: Low energy; lethargic; may be depressed despite Scale 2 score
  T<35: Likely suffering depression

Scale 0 (Si) Social Introversion — 69 items:
  T≥65: Socially introverted; insecure/uncomfortable socially; shy/timid; lacks self-confidence; overcontrolled; worries; may be depressed
  T 58–64: Prefers small groups; can interact but prefers not to
  T<40: Sociable/extroverted; outgoing; impulse control problems; may be manipulative

HARRIS-LINGOES SUBSCALES — interpret only when parent scale is elevated:
  D1 (Subjective Depression): unhappy/blue; lacks energy; nervous/tense; poor concentration; socially withdrawn
  D2 (Psychomotor Retardation): immobile/withdrawn; denies aggressive impulses
  D3 (Physical Malfunctioning): preoccupied with physical functioning; denies good health
  D4 (Mental Dullness): lacks energy; feels tense; memory/judgment difficulties
  D5 (Brooding): ruminates/cries; may feel life not worthwhile; losing control of thoughts
  Hy1 (Denial of Social Anxiety): socially extroverted; comfortable interacting
  Hy2 (Need for Affection): naively optimistic/trusting; denies negative feelings
  Hy3 (Lassitude-Malaise): feels weak/fatigued; difficulties concentrating; unhappy
  Hy4 (Somatic Complaints): multiple somatic complaints; denies hostility
  Hy5 (Inhibition of Aggression): denies hostile/aggressive impulses; sensitive to others
  Pd1 (Familial Discord): unpleasant home/family; lacks support; feels controlled
  Pd2 (Authority Problems): resents parental/societal standards; has been in trouble
  Pd3 (Social Imperturbability): comfortable/confident socially; strong opinions
  Pd4 (Social Alienation): alienated/isolated; feels misunderstood; blames others
  Pd5 (Self-Alienation): uncomfortable/unhappy; regret/guilt for past; may use alcohol
  Pa1 (Persecutory Ideas): world as threatening; misunderstood; suspicious; may have persecution delusions
  Pa2 (Poignancy): high-strung/sensitive; feels more intensely; lonely; seeks risky activities
  Pa3 (Naivete): extremely naive/optimistic about others; high moral standards; denies hostility
  Sc1 (Social Alienation): mistreated/unloved; others trying to harm; lonely/empty; avoids social situations
  Sc2 (Emotional Alienation): fear/depression/apathy; may wish to be dead; possible sadistic/masochistic needs
  Sc3 (Lack of Ego Mastery-Cognitive): fears losing mind; strange thoughts; unreality; concentration difficulties
  Sc4 (Lack of Ego Mastery-Conative): life is a strain; despair; withdraws into fantasy; given up hope
  Sc5 (Lack of Ego Mastery-Defective Inhibition): not in control of emotions; restless/hyperactive; uncontrolled episodes
  Sc6 (Bizarre Sensory Experiences): body changing strangely; hallucinations; ideas of external influence
  Ma1 (Amorality): perceives others as selfish/dishonest; feels justified acting similarly; vicarious satisfaction from manipulation
  Ma2 (Psychomotor Acceleration): accelerated speech/thought/activity; tense/restless; seeks excitement
  Ma3 (Imperturbability): denies social anxiety; impatient/irritable toward others
  Ma4 (Ego Inflation): unrealistic self-evaluation; resentful when others make demands
  Si1 (Shyness/Self-Consciousness): shy; easily embarrassed; uncomfortable in new situations
  Si2 (Social Avoidance): dislikes and avoids groups/crowds; avoids contact
  Si3 (Alienation-Self and Others): low self-esteem; self-critical; nervous/fearful; suspicious of others

CONTENT SCALES — T≥65 interpretations:
  ANX: nervous/worried/apprehensive; concentration problems; sleep disturbance; pessimistic; overwhelmed
  FRS: fearful/uneasy; specific fears and phobias
  OBS: great difficulty making decisions; rigid; compulsive behaviors; intrusive thoughts; ruminates over trivial things
  DEP: depressed/despondent; fatigue; hopeless; preoccupied with death or suicide; indecisive; guilty; lonely
  HEA: denies good physical health; preoccupied with bodily functioning; multiple somatic symptoms
  BIZ: psychotic thought processes; hallucinations (auditory/visual/olfactory); feelings of unreality; beliefs others control mind or say bad things
  ANG: angry/hostile; irritable/grouchy/impatient/stubborn; temper tantrums; may be physically abusive
  CYN: others seen as dishonest/selfish/uncaring; suspicious; guarded; hostile; untrusting
  ASP: trouble with school/law; believes in circumventing laws; cynical; resents authority; antisocial behaviors
  TPA: hard-driving; never enough time; dislikes waiting/interruptions; hostile/irritable; holds grudges
  LSE: poor self-concept; anticipates failure; oversensitive to criticism; passive; worries; difficulties deciding
  SOD: shy and socially introverted; prefers being alone; dislikes parties; does not initiate conversations
  FAM: considerable family discord; families lacking love/support; angry toward family; marital unhappiness
  WRK: wide variety of attitudes/behaviors contributing to poor work performance; obsessive; poor concentration; tense/fearful; gives up easily — NOTE: T≥95 on WRK indicates SEVERE work functioning impairment, not merely dissatisfaction
  TRT: negative attitudes toward mental health professionals; feels no one understands; gives up easily; unable to make changes

PSY-5 SCALES — T≥65:
  AGGR: offensive/instrumental aggression; intimidates others; dominance/hate; desire for power; likely violent/cruel
  PSYC: disconnection from reality; delusions of reference; disorganized/bizarre thinking; perceptual aberrations; magical ideation
  DISC: risk-taking; impulsive; easily bored; rule-breaking; higher criminality; ignores moral codes
  NEGE: focuses on problematic information; worries; self-critical; guilty; nervous; worst-case scenarios; negative affect
  INTR T≥65: little joy or positive engagement; low positive emotionality
  INTR T<40: seeks and enjoys social experiences; energetic; engaged

SUPPLEMENTARY SCALES:
  Es (Ego Strength): T≥65 = stable/reliable/resourceful; tolerates confrontation; T<40 = poor self-esteem; poor daily functioning; poor therapy prognosis
  MAC-R: T≥65 = addictive personality; strong likelihood substance abuse; T 55–65 = suggestive but many false positives; T<55 = contraindicated
  A (Anxiety): T≥65 = anxious/uncomfortable; slow tempo; pessimistic; apathetic; inhibited; confused under stress; motivated for therapy once trust established
  R (Repression): T≥58 = submissive; conventional; painstakingly careful; avoids unpleasantness; unwilling to discuss problems
  Mt (College Maladjustment): T≥65 = ineffectual; pessimistic; anxious and worried; procrastinates; somatic complaints; feels life is a strain — IMPORTANT: Mt measures GENERAL MALADJUSTMENT AND DISTRESS, NOT manic or hypomanic symptoms. Never describe Mt as indicating mania or hypomania.
  OH (Overcontrolled Hostility): T≥65 = fewer angry feelings; socialized/responsible; appropriate most of the time but occasional exaggerated aggressive responses
  Do (Dominance): T≥65 = poised/self-assured; leadership; T<40 = submissive; lacks confidence; easily influenced
  Re (Social Responsibility): T≥65 = deep concern for ethical/moral issues; strong justice sense; less likely criminal; T<40 = unwilling to accept responsibility; low integrity
  PK (PTSD-Keane): T≥65 = intense emotional distress; sleep disturbances; guilt/depression; unwanted disturbing thoughts; fears loss of cognitive/emotional control; feels misunderstood
  GM (Masculine Gender Role): T≥65 males = high self-confidence; persistence; wide interests; lack of fears — EXPERIMENTAL, interpret with caution
  GF (Feminine Gender Role): T≥65 males = religiosity; avoidance of swearing; frankness; bossiness; poor temper control; susceptibility to substance abuse — EXPERIMENTAL, interpret with caution
  APS/AAS: T≥65 = substance abuse likelihood; AAS absence of elevation does NOT rule out substance abuse
  MDS (Marital Distress): T≥65 = significant marital relationship problems
  Ho (Cook-Medley Hostility): T≥65 = chronic hostility; cynicism; suspicious of others' motives

HIGH-POINT PAIR INTERPRETATIONS — use these when pair scores are both T≥65:
  2-7/7-2: Worry, depression, pessimism with anxiety, tension, lack of self-confidence; guilt-ridden; obsessively preoccupied with deficiencies; if either scale >80 consider pharmacotherapy before psychotherapy; excellent prognosis if distress manageable; most likely diagnosis: depressive or anxiety disorder
  7-4/4-7: Cyclical acting out followed by guilt/regret; controls insufficient to prevent recurrences; may include alcohol/drug abuse; irritated by rules; callous to others' feelings; guarded prognosis
  1-2/2-1: Depression, worry, pessimism with many somatic complaints; passive-dependent; cynical about treatment; poor motivation for change; seeks medical attention
  1-3/3-1: Immature/egocentric; hysteroid; conversion symptoms; overcontrolled; lacks insight; poor therapy candidate; denies psychological causation
  2-4/4-2: Either psychopathic individuals reacting to constraints, or extremely hostile/angry with marital/family turmoil; impulse control problems; characterological difficulties
  4-8/8-4: Irritable/hostile/suspicious; projection and acting out; schizoid; socially isolated; poor empathy; poor judgment; suicidal attempts relatively common; borderline, schizoid, or schizophrenia diagnoses
  7-8/8-7: Chronic personality difficulties; excessive worry; passivity; dependency; insecurity; inferiority; rich fantasy life; sexual identity concerns; formal thought disorder possible; poor therapy prognosis
  2-8/8-2: Suicidal ideation likely; self-destructive potential high; confusion; retarded thinking; stereotyped problem solving; major depression, schizophrenia, or schizoaffective diagnoses most likely; prognosis poor
  4-9/9-4: Impulsive/irresponsible; shallow/superficial relationships; poor judgment; antisocial personality most likely diagnosis

CRITICAL ITEMS — MANDATORY REPORTING RULES:
- If Koss-Butcher Depressed Suicidal Ideation items are endorsed: YOU MUST explicitly address suicide risk in Section A (validity) AND in the clinical formulation. State the number of items endorsed and that clinical risk assessment is required. Do not bury this finding or minimize it.
- If Threatened Assault items are endorsed: YOU MUST address dangerousness and duty-to-warn considerations.
- Critical items are listed for hypothesis generation only — single items are unreliable but clusters require clinical attention.
...
Write a comprehensive professional MMPI-2 assessment report with exactly these sections:
...
If Fp is elevated ≥70, this MUST be prominently discussed. If suicidal ideation critical items are endorsed,
state this here and require clinical follow-up. Do not minimize or omit any validity concerns.
...
Address ALL clinically elevated scales (T≥65) individually.
Use the correct behavioral correlates and diagnostic considerations from the rules above.
Do NOT confuse scale names or descriptions — e.g., Mt is maladjustment NOT mania.
...
Use subscale findings to clarify which facets of the parent scale are driving the elevation.

SECTION D — CONTENT SCALE ANALYSIS
Discuss all elevated content scales using the descriptions above.
WRK at T≥90 must be described as severe work functioning impairment, not merely dissatisfaction.
DEP elevation with suicidal ideation critical items must cross-reference the risk finding.

SECTION E — PSY-5 AND SUPPLEMENTARY SCALES
Discuss elevated PSY-5 scales and key supplementary findings using the descriptions above.
...
If suicidal ideation critical items were endorsed, explicitly state that risk assessment is required.
Note specific diagnostic hypotheses with reference to supporting scale configurations.

SECTION G — TREATMENT IMPLICATIONS
Evidence-based treatment recommendations. Note prognosis based on scale configurations.
If high-point pair has poor therapy prognosis, state this explicitly.

SECTION H — SUMMARY
One concise paragraph summarizing the profile for clinical records.
Include any risk alerts prominently.

Use formal clinical language. Reference specific T-scores throughout.
...
def create_pdf(path, client_name, age, gender, scores, validity, report_text,
    ...
    # Client info — 4-row expanded table
    info_data = [
    ...
    if st.session_state.submitted:
        ...
        if st.session_state.current_page == 0:
            ...
            with col2:
                ...
                nationality = st.text_input("Nationality", placeholder="e.g., Egyptian",
            ...
            with col3:
                ...
                referral = st.text_input("Referral Source", placeholder="e.g., Dr. Ahmed Hassan",
